...
class SelfAttention(nn.Module):
    def __init__(self, embedding_dim, heads):
        ...
        assert(self.head_dim * heads == embedding_dim), "Embedding dimension needs to be divisible by heads"
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, heads, device, forward_expansion, dropout, max_length):
        ...
        self.layers = nn.ModuleList([
            TransformerBlock(embedding_dim, heads, dropout=dropout, forward_expansion=forward_expansion) for _ in range(num_layers)
        ])
        self.dropout = nn.Dropout(dropout)
...
class Decoder(nn.Module):
    def __init__(self, tgt_vocab_size, embedding_dim, num_layers, heads, forward_expansion, dropout, device, max_length):
        super(Decoder, self).__init__()
        self.device = device
        self.word_embedding = nn.Embedding(tgt_vocab_size, embedding_dim)
        self.position_embedding = nn.Embedding(max_length, embedding_dim)
        self.layers = nn.ModuleList([
            DecoderBlock(embedding_dim, heads, forward_expansion, dropout, device) for _ in range(num_layers)
        ])
